...
def evaluate_candle(smart, option_lookup, gc, sh, underlying, index_token, strike_step, expiry_date, active_positions, daily_trade_counts, last_processed, trading_date):
    ...
    if can_enter and signal:
        ...
        if token is None:
            logging.error(f"[{underlying}] Could not resolve option token for symbol '{symbol}' — check expiry/strike format against option_lookup keys. Signal was {signal} at {ts}, entry skipped.")
            return
        ...
        if opt_df.empty:
            for opt_fetch_attempt in range(1, DATA_FRESHNESS_RETRIES + 1):
                logging.warning(f"[{underlying}] Option candle fetch empty for {symbol} (attempt {opt_fetch_attempt}/{DATA_FRESHNESS_RETRIES}) — cannot enter yet, retrying in {DATA_FRESHNESS_RETRY_DELAY}s...")
                time.sleep(DATA_FRESHNESS_RETRY_DELAY)
                now = get_ist_now()
                opt_df, smart = get_candles_with_relogin(smart, token, day_from_warmup, now, exchange="NFO", interval=INTERVAL)
                if not opt_df.empty:
        ...
        if opt_df.empty:
            logging.error(f"[{underlying}] GAVE UP entering {symbol} — option candle data still empty after {DATA_FRESHNESS_RETRIES} retries. Signal was {signal} at {ts}, entry skipped this cycle.")
...
def run_live_trading_engine(stop_event, sheet_id):
    ...
    send_telegram_alert("🚀 *Pro Engine v7.4 Started - Live Trading Restricted to NIFTY*")
